...
async function analyzeWithGroq(base64Image, groqApiKey) {
  const prompt = `Ты — система родительского контроля CyberParent AI. Анализируй скриншот экрана ребёнка и определяй только РЕАЛЬНЫЕ угрозы.

    ⚠️ ВАЖНО: Никогда не помечай как угрозу:
    - Сам интерфейс CyberParent AI (окна с надписями "CyberParent", "мониторинг", "блокировка")
    - Системные окна Windows
    - Обычные программы (браузер, игры, офис)
    - Настройки системы
    - Любые легитимные действия пользователя

    🔴 РЕАЛЬНЫЕ угрозы для блокировки (risk 70-100):
    - Фишинговая страница банка (поддельный адрес, запрос CVV/пароля)
    - Вредоносный сайт с явным требованием скачать .exe/.scr
    - Кража данных: запрос паспорта, SMS-кода, полных реквизитов карты
    - порнография или экстремизм

    🟡 Предупреждение (risk 30-69):
    - Подозрительная форма ввода пароля на незнакомом сайте
    - Всплывающее окно с запугиванием ("вирус найден, позвоните...")
    - Подозрительная реклама

    🟢 Безопасно (risk 0-29):
    - Всё остальное: обычный сёрфинг, соцсети, игры, учёба, YouTube

    Верни ТОЛЬКО JSON (без markdown, без пояснений):
    {
      "risk": число 0-100,
      "threat_type": "phishing|credential_theft|malware|safe",
      "reason": "краткое описание (максимум 50 символов, на русском)",
      "location": "где на экране (центр, верх, форма ввода)"
    }

    Если скриншот не содержит явных угроз — риск должен быть низким (0-20).`;
